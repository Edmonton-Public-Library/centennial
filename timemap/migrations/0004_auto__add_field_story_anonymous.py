...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Branch', fields ['start_year']
        db.create_index('timemap_branch', ['start_year'])

        # Adding index on 'Branch', fields ['end_year']
        db.create_index('timemap_branch', ['end_year'])

        # Adding index on 'Branch', fields ['name']
        db.create_index('timemap_branch', ['name'])

        # Adding field 'Story.anonymous'
        db.add_column('timemap_story', 'anonymous',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        # Adding index on 'Story', fields ['description']
        db.create_index('timemap_story', ['description'])

        # Adding index on 'Story', fields ['title']
        db.create_index('timemap_story', ['title'])

        # Adding index on 'Story', fields ['year']
        db.create_index('timemap_story', ['year'])

        # Adding index on 'Story', fields ['content_type']
        db.create_index('timemap_story', ['content_type'])


    def backwards(self, orm):
        # Removing index on 'Story', fields ['content_type']
        db.delete_index('timemap_story', ['content_type'])

        # Removing index on 'Story', fields ['year']
        db.delete_index('timemap_story', ['year'])

        # Removing index on 'Story', fields ['title']
        db.delete_index('timemap_story', ['title'])

        # Removing index on 'Story', fields ['description']
        db.delete_index('timemap_story', ['description'])

        # Removing index on 'Branch', fields ['name']
        db.delete_index('timemap_branch', ['name'])

        # Removing index on 'Branch', fields ['end_year']
        db.delete_index('timemap_branch', ['end_year'])

        # Removing index on 'Branch', fields ['start_year']
        db.delete_index('timemap_branch', ['start_year'])

        # Deleting field 'Story.anonymous'
        db.delete_column('timemap_story', 'anonymous')
